...
\begin{abstract}
Predicting the short-term trajectory of mid-prices in a Limit Order Book (LOB) is a profoundly difficult financial engineering problem due to extreme non-stationarity, microstructure noise, and imbalanced class distributions. This comprehensive project introduces an advanced Mixture-of-Experts (MoE) machine learning pipeline designed to execute profitable High-Frequency Trading (HFT) decisions on the FI-2010 benchmark dataset. We rigorously explore the ``Classification-Profitability Paradox''---demonstrating that models optimizing heavily for theoretical mathematical accuracy often fail entirely in financial execution due to unmodeled spread-crossing constraints and low-conviction thresholds. By assembling diverse algorithmic specialists---an L2-Regularized Logistic Regression, a Gradient-Boosted Decision Tree (XGBoost), and a Temporal Multi-Layer Perceptron (MLP) mapping sequential data via Focal Loss---and orchestrating them via an overarching Gating Network coupled with a 20-step signal regime execution filter, our architectural framework achieves a mean return of +46.5\% across 9 chronological folds. While this performs comparably to the best single expert (XGBoost, +43.6\%), the difference is not statistically significant (paired t-test, p = 0.38), highlighting the central challenge that predictive accuracy alone is insufficient for profitable decision-making under transaction costs and execution constraints.
\end{abstract}
...
\subsection{The Mixture of Experts Outcome}
The MoE Gating architecture demonstrates the ability to combine expert signals. The Gating algorithm learned to weight the trees' directional conviction while utilizing the MLP's Stationarity bias to block noisy edge-cases. The resulting framework achieved a mean return of $+46.5\%$, which performs comparably to XGBoost solo ($+43.6\%$). However, a paired t-test across 9 folds yields $p = 0.38$, indicating no statistically significant improvement. The MoE wins in only 2 of 9 folds, and the median return is lower than XGBoost's. This suggests that while the gating mechanism shows potential benefit over naive ensembles, it does not consistently improve upon the best single expert in this dataset.

\section{Conclusion}
This framework illustrates that limit order book predictive architectures cannot be optimized solely via abstract classification limits. By combining diverse algorithmic priors and enforcing structural hysteresis buffers, the MoE pipeline achieves competitive performance. However, the MoE does not show statistically significant improvement over the best single model (XGBoost), reinforcing the central finding: predictive accuracy alone is insufficient for profitable decision-making under transaction costs and execution constraints. Performance differences across 9 chronological folds remain within variance ($p = 0.38$).
